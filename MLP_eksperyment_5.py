...
for max_epoch in epoch_values:
    ...
    for lr in learning_rates:
        ...
        for i, (train, test) in enumerate(skfold.split(data, np.squeeze(target)), start=0):
            ...
            print("Epoch #{:<4} Test #{:<2}: PK_vec {} test_size {}".format(max_epoch, i, PK, n_test_samples))

        PK_mean_vec.append(np.mean(PK_vec))
        print("Epoch: {}, Learning Rate: {}, Mean PK: {}".format(max_epoch, lr, np.mean(PK_vec)))

    PK_mean_epoch_vec.append(np.mean(PK_mean_vec))

print("Execution time:", timer() - start)

# Plot
plt.plot(epoch_values, PK_mean_epoch_vec, marker='o')
