...
lr_values = [0.0001, 0.001, 0.0003]
epoch_values = np.arange(10, 1801, 400)
...
for lr in lr_values:
    ...
    for epoch in epoch_values:
        W = lvqTraining(data, target, lr=lr, max_epoch=epoch, err_goal=1e-10)
        predicted = []
        for sample in data:
        ...
        PK_values.append(PK)
    plt.plot(epoch_values, PK_values, label=f'LR={lr}')

plt.xlabel('Epochs')
plt.ylabel('Classification Accuracy (PK)')
plt.title('Epochs vs Classification Accuracy for Different Learning Rates')
plt.legend()
plt.grid(True)
plt.show()
