...
lr_values = []
PK_values = []
for lr in np.arange(0.001, 0.00005, -0.0001):
    W = lvqTraining(data, target, lr=lr, max_epoch=500, err_goal=1e-10)
    predicted = []
    for sample in data:
    ...
    lr_values.append(lr)
    PK_values.append(PK)
plt.plot(lr_values, PK_values)
plt.xlabel('Learning Rate (lr)')
plt.ylabel('Classification Accuracy (PK)')
plt.title('Learning Rate vs Classification Accuracy')
plt.show()
