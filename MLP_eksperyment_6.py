...
import numpy as np
import nnet_jit4 as net  # Assuming nnet_jit4 is the correct module name
...
epoch_values = [10, 50, 100, 500, 1000, 2000]
...
for lr in learning_rates:
    ...
    for max_epoch in epoch_values:
        ...
        for i, (train, test) in enumerate(skfold.split(data, np.squeeze(target)), start=0):
            ...
            print("Learning Rate: {}, Epoch #{:<4} Test #{:<2}: PK_vec {} test_size {}".format(lr, max_epoch, i, PK, n_test_samples))

        PK_mean_vec.append(np.mean(PK_vec))
        print("Learning Rate: {}, Epoch: {}, Mean PK: {}".format(lr, max_epoch, np.mean(PK_vec)))

    PK_mean_epoch_vec.append(PK_mean_vec)

print("Execution time:", timer() - start)

# Plot
for i, lr in enumerate(learning_rates):
    plt.plot(epoch_values, PK_mean_epoch_vec[i], marker='o', label='LR={}'.format(lr))

plt.xlabel('Number of Epochs')
plt.ylabel('Mean PK')
plt.title('Mean PK vs. Number of Epochs')
